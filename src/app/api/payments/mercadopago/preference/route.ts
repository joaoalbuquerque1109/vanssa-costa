...
export async function POST(request: Request) {
  ...
  try {
    preferenceData = (await preference.create({
      body: {
        ...
        external_reference: externalReference,
        payer: payload.payerEmail || payload.customer?.email ? { email: payload.payerEmail ?? payload.customer?.email ?? undefined } : undefined,
        notification_url: notificationUrl,
        back_urls: backUrls,
  ...
  return NextResponse.json({
    ...
    external_reference: externalReference,
    checkout_url: preferenceData.init_point ?? preferenceData.sandbox_init_point,
